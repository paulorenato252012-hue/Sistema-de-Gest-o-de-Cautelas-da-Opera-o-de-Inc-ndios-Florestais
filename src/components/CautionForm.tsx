...
import { Save, Plus, Trash2, PackagePlus, CheckCircle2, AlertCircle, Sparkles, Wrench, RefreshCw, Camera, X, Eye, ShoppingBag } from 'lucide-react';
...
export function CautionForm({ existingCaution, existingItems = [], onSaved }: CautionFormProps) {
  ...
  const [uploadingCestaImage, setUploadingCestaImage] = useState(false);
  const [previewImage, setPreviewImage] = useState<string | null>(null);

  const handleCestaImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputElement = e.target;
    if (!inputElement.files || inputElement.files.length === 0) return;
    const file = inputElement.files[0];
    
    setUploadingCestaImage(true);
    try {
      const url = await uploadAvariaImage(file, 'cesta_' + Date.now());
      const currentPhotos = formData.cestaBasicaFotos || [];
      handleInputChange('cestaBasicaFotos', [...currentPhotos, url]);
    } catch (err) {
      console.error("Erro ao processar a imagem da cesta:", err);
      alert("Não foi possível carregar a fotografia. Tente novamente.");
    } finally {
      setUploadingCestaImage(false);
      inputElement.value = '';
    }
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (formData.type === 'CESTA_BASICA') {
        if (!formData.cestaBasicaFotos || formData.cestaBasicaFotos.length === 0) {
          setError('O registro fotográfico da Cesta Básica é obrigatório para liberar a assinatura. Anexe pelo menos uma foto.');
          setSaving(false);
          return;
        }
        if (!formData.cestaBasicaQtd || formData.cestaBasicaQtd < 1) {
          setError('A quantidade de Cestas Básicas recebidas é obrigatória.');
          setSaving(false);
          return;
        }
        if (!formData.cestaBasicaVolumes || formData.cestaBasicaVolumes < 1) {
          setError('O número de volumes da Cesta Básica é obrigatório.');
          setSaving(false);
          return;
        }
      }
    ...
    <div className="space-y-8">
      ...
      {!existingCaution && (
        <div className="bg-white p-2 rounded-lg border border-gray-200 shadow-sm flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={() => handleInputChange('type', 'CESTA_BASICA')}
            className={`flex-1 py-2.5 px-4 text-sm font-semibold rounded-md transition-all text-center ${
              formData.type === 'CESTA_BASICA'
                ? 'bg-red-800 text-white shadow-sm'
                : 'text-gray-700 hover:bg-gray-100'
            }`}
          >
      ...
      {formData.type !== 'CESTA_BASICA' ? (
        ...
        <div className="bg-emerald-50/60 p-5 rounded-lg border border-emerald-200 shadow-sm space-y-4">
          <h3 className="text-lg font-bold text-gray-900 flex items-center">
            <ShoppingBag className="w-5 h-5 mr-2 text-emerald-800" />
            Dados da Cesta Básica
          </h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">
                Quantidade de Cestas Recebidas *
              </label>
              <input
                type="number"
                min="1"
                readOnly={isReadOnly}
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-emerald-500 focus:border-emerald-500 bg-white"
                value={formData.cestaBasicaQtd || ''}
                onChange={e => handleInputChange('cestaBasicaQtd', Number(e.target.value))}
                placeholder="Ex: 50"
              />
            </div>
            <div>
              ...
              <input
                ...
                readOnly={isReadOnly}
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-emerald-500 focus:border-emerald-500 bg-white"
                value={formData.cestaBasicaVolumes || ''}
                onChange={e => handleInputChange('cestaBasicaVolumes', Number(e.target.value))}
                placeholder="Ex: 100"
              />
            </div>
            <div className="md:col-span-2">
              ...
              <textarea
                readOnly={isReadOnly}
                rows={2}
                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-emerald-500 focus:border-emerald-500 bg-white resize-none"
                value={formData.cestaBasicaObservacoes || ''}
                onChange={e => handleInputChange('cestaBasicaObservacoes', e.target.value)}
                placeholder="Ex: Algumas caixas vieram levemente amassadas..."
              />
            </div>
          </div>

          <div className="border-t border-emerald-200/60 pt-4 mt-2">
            <div className="flex items-center justify-between mb-2">
              <div>
                <h4 className="text-sm font-bold text-gray-900">Registro Fotográfico Obrigatório *</h4>
                <p className="text-xs text-gray-600">Para liberar a assinatura, anexe ao menos uma foto das cestas recebidas.</p>
              </div>
              {!isReadOnly && (
                <label className="cursor-pointer px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded shadow-sm flex items-center transition-colors">
                  {uploadingCestaImage ? (
                    <RefreshCw className="w-4 h-4 mr-1.5 animate-spin" />
                  ) : (
                    <Camera className="w-4 h-4 mr-1.5" />
                  )}
                  {uploadingCestaImage ? 'Enviando...' : 'Fotografar'}
                  <input
                    type="file"
                    accept="image/*"
                    capture="environment"
                    className="hidden"
                    onChange={handleCestaImageUpload}
                    disabled={uploadingCestaImage}
                  />
                </label>
              )}
            </div>

            {formData.cestaBasicaFotos && formData.cestaBasicaFotos.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-3">
                {formData.cestaBasicaFotos.map((url, idx) => (
                  <div key={idx} className="relative group">
                    <img 
                      src={url} 
                      alt={`Cesta Básica ${idx + 1}`} 
                      onClick={() => setPreviewImage(url)}
                      className="w-20 h-20 object-cover rounded-lg border border-gray-300 cursor-pointer shadow-sm hover:opacity-85 transition"
                    />
                    {!isReadOnly && (
                      <button
                        type="button"
                        onClick={() => {
                          const newPhotos = [...formData.cestaBasicaFotos!];
                          newPhotos.splice(idx, 1);
                          handleInputChange('cestaBasicaFotos', newPhotos);
                        }}
                        className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1 hover:bg-red-700 shadow-md transition"
                        title="Remover foto"
                      >
